...
/*		public function getCustomer() {
		...
		public static function priceFormat ($price) {
			...
			*/
			if ( Shop::module()->preCurrencySymbol )
		...
		public static function getCartContent() {
			//print_r(Yii::app()->user->getState('cart'));
		...
		public static function getNumberCart() {
			return count(Shop::getCartContent());
		}
		
		public static function getPriceTotal($store_id = null) {
			$price_total = 0;
			$tax_total = 0;
			$products = Shop::getCartContent();
			
			foreach($products[$store_id] as $product)  {
			...
			/*	
			$price_totalx = Shop::t('Price total: {total}', array(
						'{total}' => Shop::priceFormat($price_total),
						)); 
			$price_totalx .= '<br />';
			$price_totalx .= Shop::t('All prices are including VAT: {vat}', array(
						'{vat}' => Shop::priceFormat($tax_total))) . '<br />';
			*/
			$price_totalx = '<table class="shipping_produk" width="100%" cellpadding="0" cellspacing="0">';
			$price_totalx .= '<tr>';
			$price_totalx .= '<td width="50%">';
			$price_totalx .= 'Total';
			$price_totalx .= '</td>';
			$price_totalx .= '<td  width="50%" style="text-align:right;">';
			$price_totalx .= Shop::priceFormat($price_total);
			$price_totalx .= '</td>';
			$price_totalx .= '</tr>';
			$price_totalx .= '</table>';
			$price_totalx .= '<div class="kotak-info-shipping">'.Shop::t('Harga belum termasuk biaya pengiriman') . '</div>';
			return $price_totalx;
		}
	...
	public static function module()
	{
		...
		/*  11/12/2012 */
		public static function getAdditionalFee($orderId){
			return $addFee = ShopAdditionalPrice::model()->findAll("order_id = '".$orderId."'");
		}
