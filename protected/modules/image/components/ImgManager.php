...
class ImgManager extends CApplicationComponent
{
	 ...
	 * @param string $group the group collection of image.
	 * @param string $group the group collection of image.
	 * @return Image the image record.
	 * @throws ImageException if saving the image record or file fails.
	 */
	public function save($file,$name=null,$path=null, $group=null,$container_id = null)
	{
		...
		{
			switch ($group){
				case 'avatar_company': // only one in emage, should be delete
				case 'avatar': // only one in emage, should be delete
					$image= Allimage::model()->find('group_image = :group AND container_id = :cid',
								array(':group'=>$group,':cid'=>$container_id));
					//echo $image->id;
					//die();
					if ($image->id)
						$oldImage = $image->id;
	 ...
	 * @return object path
	 */
	public function createVersionForPath($id,$version)
	{
		if(isset($this->versions[$version]))
		{
			$image=$this->loadModel($id);
			...
			if($image!=null)
			{
				$path		= $this->resolveImageVersionPath($image,$version);
				$fileName	= $this->resolveFileName($image);
				$urlpath = $image->path.'/'.$version.'/'.$fileName;
	...
	{
		/** @var Image $image */
		$image = Allimage::model()->findByPk($id);

		if($image instanceof Allimage)
		{
	...
	{
		return Allimage::model()->findByPk($id);
	}
	...
	protected function resolveImageVersionPath($image,$version)
	{
		...
		return $this->getVersionPath($image->getPath().$version,true);
	}
	...
	protected function getVersionBasePath($absolute=false)
	{
		...
		if($this->_versionBasePath!==null)
			...
			$path.=$this->_versionBasePath = $this->getImagePath();
	...
	public function renderVersion($id,$version)
	{
		$versions=Yii::app()->image->versions;
		if(isset($versions[$version]))
		{
			return $thumb = Yii::app()->image->createVersionForPath($id,$version);
			//$thumb->render();
		}
		else
