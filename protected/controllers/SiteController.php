...
class SiteController extends Controller
{
	...
	public function actionLogin()
	{
		...
		if(isset($_POST['LoginForm']))
		{
			...
			if( $model->validate() && $model->login()){
				...
				$this->redirect(Yii::app()->createUrl('members/'.$model->username));
			}else{
	...
	public function actionSignup()
	{
		...
		if(isset($_POST['SignupForm']))
		{
			...
			// validate user input and redirect to the previous page if valid
			if( $model->validate() && $newMembers = $model->save() ){
				
				$member_profile = new Profile();
				$member_profile->firstname = $_POST['SignupForm']['first_name'];
				$member_profile->lastname = $_POST['SignupForm']['last_name'];
				$member_profile->user_id = $newMembers->id;
				$member_profile->save();
				$message = new YiiMailMessage();
