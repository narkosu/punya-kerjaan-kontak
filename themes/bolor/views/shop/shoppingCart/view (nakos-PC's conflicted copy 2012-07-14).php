...
if($products) {
	foreach($products as $store_id => $store_products) {
		echo '<div class="title">'.Shopstore::model()->findByPk($store_id)->shop_name.'</div>';
		...
		foreach($store_products as $position => $product) {
			...
			if(@$model = Products::model()->findByPk($product['product_id'])) {
				...
				printf('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td class="text-right">%s</td><td class="text-right price_'.$position.'">%s</td><td>%s</td></tr>',
						...
						CHtml::textField('amount_'.$store_id.'_'.$position,
							$product['amount'], array(
								'size' => 4,
								'class' => 'amount_'.$store_id.'_'.$position,
								)
							),
						$model->title,
						$variations,
				...
				Yii::app()->clientScript->registerScript('amount_'.$store_id.'_'.$position,"
						$('.amount_".$store_id.'_'.$position."').keyup(function() {
							$.ajax({
								url:'".$this->createUrl('//shop/shoppingCart/updateAmount')."',
								data: $('#amount_".$store_id.'_'.$position."'),
								success: function(result) {
								$('.amount_".$store_id.'_'.$position."').css('background-color', 'lightgreen');
