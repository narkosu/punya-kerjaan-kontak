	...
	<?php if($this->getAction()->getId()!='sents') : ?>
		<div style="clear:left"> <span class="mailbox-buttons-label">With selected:</span> 
				<?php if($this->getAction()->getId()=='trash') : ?>
					<input type="submit" id="mailbox-action-restore" class="btn mailbox-button button smallrounded" name="button[restore]" value="restore" /> 
					<input type="submit" id="mailbox-action-delete" class="btn mailbox-button  button smallrounded" name="button[delete]" value="delete forever" />
				<?php else: ?>
					<?php if(!$this->module->readOnly || ( $this->module->readOnly && !$this->module->isAdmin()) ): ?>
						<input type="submit" id="mailbox-action-delete" class="btn mailbox-button  button smallrounded" name="button[delete]" value="delete" /> 
					<?php endif; ?>
						<input type="submit" id="mailbox-action-read" class="btn mailbox-button  button smallrounded" name="button[read]" value="read" /> 
						<input type="submit" id="mailbox-action-unread" class="btn mailbox-button  button smallrounded" name="button[unread]" value="unread" /> 
				<?php endif; ?>
		</div>
	<?php endif; ?>
	</div>
